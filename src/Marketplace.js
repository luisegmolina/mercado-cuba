...
import { ShoppingBag, Store, MessageCircle, ChevronLeft, Search } from 'lucide-react';
  ...
  return (
    <div className="min-h-screen bg-gray-900 pb-20 text-white">
      ...
      <div className="sticky top-0 bg-gray-900/95 backdrop-blur-md z-30 px-4 py-4 border-b border-gray-800 flex items-center gap-3">
        <button onClick={() => navigate('/')} className="p-2 -ml-2 rounded-full hover:bg-gray-800 text-gray-400">
          <ChevronLeft />
        </button>
      ...
      <div className="px-4 grid grid-cols-2 gap-4">
        {filtered.map((p) => (
          <div key={p.id} className="bg-gray-800 rounded-2xl overflow-hidden border border-gray-700 flex flex-col shadow-2xl">
            <div className="aspect-square bg-gray-900 relative">
                {/* Reutiliza tu componente SafeImage si lo exportas */}
                <img src={JSON.parse(p.image_url)[0]} alt={p.name} className="w-full h-full object-cover" />
                <div className="absolute top-2 left-2 bg-black/60 backdrop-blur-md px-2 py-1 rounded-lg flex items-center gap-1.5 border border-white/10">
                    ...
                    <span className="text-[10px] font-bold truncate max-w-[80px]">{p.store_name}</span>
                </div>
            </div>
            <div className="p-3 flex-1 flex flex-col gap-2">
              <h3 className="text-xs font-semibold text-gray-200 line-clamp-2">{p.name}</h3>
              <div className="mt-auto">
                <p className="text-blue-400 font-black text-sm">{p.price_cup} CUP</p>
                <button 
                  onClick={() => window.open(`https://wa.me/${p.store_whatsapp}`)}
